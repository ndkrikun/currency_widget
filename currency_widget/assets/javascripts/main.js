...
class Widget extends Component {
  ...
  renderPart(data, type) {
    const field = type === 'input'
      ? ( <input className="widget__field" type="text" placeholder="Type value" 
          value={this.state.inputVal} 
          onChange={this.handleAmmountChange.bind(this)} />
        )
      : ( <div className="widget__field">{this.state.outputVal}</div> )

    return (
      <div className="widget__part">
        <div className={`widget__currency ${data.opened ? 'is-opened' : ''}`}>
          {data.items.map(item => this.renderCurrency(item, type) )}
          <span className="widget__currency-arrow"
            onClick={() => this.toggleCurrency(type)}>
          </span>
        </div>
        {field}
      </div>
